...
const BACKEND_BOOKS_URL = 'http://localhost:4000/books';

export const getBooks = ({page, per_page}) => {
    return dispatch => {
        dispatch({type: GET_ALL_BOOKS_REQUEST});
        return axios.get(
            `${BACKEND_BOOKS_URL}/?page=${page}&per_page=${per_page}`,
...
export const addBook = book => {
    return dispatch => {
        ...
        return axios.post(`${BACKEND_BOOKS_URL}/new`, book,
            {
                headers: {
                    'Content-Type': 'application/json'
                }
...
export const sortByParameterAndMethod = ({sortItem, sortMethod}) => {
    return (dispatch, getState) => {
        const {book} = getState();
                ...
                return axios.get(
                    `${BACKEND_BOOKS_URL}/?page=${book.pagination.page}&per_page=${book.pagination.per_page}&sortItem=${sortItem}&sortMethod=${sortMethod}`,
                    {headers: {'Content-Type': 'application/json'}
                    })
                    .then(json => {
                        if(json.status === 200) {
